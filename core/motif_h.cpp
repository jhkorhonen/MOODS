...
vector<double> expected_scores(const score_matrix &mat, const vector<double> &bg, unsigned int a, unsigned int q){

    const bits_t SHIFT = MOODS::misc::shift(a);
    const bits_t MASK = (1 << (SHIFT)) - 1;

    vector<double> ret(mat[0].size(), 0);
    for (int i = 0; i < mat[0].size(); ++i){
        for (int j = 0; j < mat.size(); ++j){
            double bg_prop = 1;
            for (k = 0; k < q; ++k){
                bg_prop *= bg[MASK & (j >> (SHIFT * k))];
            }
...
MotifH::MotifH (const score_matrix& matrix, const vector<double>& bg, unsigned int window_size, double threshold, unsigned int alphabet_size, unsigned int dep_len)
{
    ...
    vector<double> es = expected_scores(mat, bg, l, q);
